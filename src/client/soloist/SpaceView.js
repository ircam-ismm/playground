...
class SpaceView extends View {
  ...
  renderPoint(point, $shape = null) {
    if ($shape === null) {
      ...
      $dot.setAttribute('r', point.radius || 8);
      ...
      if (Number.isInteger(point.index)) {
        ...
        $text.setAttribute('transform', `translate(${point.radius || 8}, -${point.radius || 8})`);

        const $textNode = document.createTextNode(point.index);
    ...
    $shape.setAttribute('transform', `translate(${point.x * this.ratio}, ${point.y * this.ratio})`);

    if (point.color) {
      $dot.style.fill = point.color;
    }
